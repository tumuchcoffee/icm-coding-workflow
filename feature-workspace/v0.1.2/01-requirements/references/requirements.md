...
* Header: the header component will have, from left to right, a hamburger icon that drops down the main menu, the application title as an H1 tag with the text "Synergistic". Each of these is floated to the left. Floating to the far right is a user profile icon that links, for now, to the root url. It will get a new link once the user profile code is written. The header component will be "sticky" and not scroll with the page.
...
* Footer: the default content in the footer will be a centered H3 tag with the text "Synergistic" that will be sticky and not scroll with the page.
