...
**Goal:** Produce a single consolidated requirements draft at `.\output\*.md` from all source material in `.\references\`.
...
2. **Extract all requirements signals.** For every source file, pull out:
   ...
   - Explicit functional requirements (user stories, use cases, acceptance criteria, feature descriptions).
   - Explicit non-functional requirements (performance, security, compliance, availability, scalability, usability thresholds — anything with a number or a measurable target).
...
5. **Write the draft.** Save the consolidated output to `.\output\feature-requirements-draft.md` using this skeleton:

   - `# Feature Requirements Draft` — title and date.
   - `## Classification` — your tier assessment from Section 1 (include the scored matrix).
   - `## Stakeholders` — who was referenced or identified.
   - `## Functional Requirements` — grouped, SMART where the tier demands it, with source attributions.
   - `## Non-Functional Requirements` — quantified thresholds, with source attributions.
   - `## Open Questions & Conflicts` — contradictions flagged during extraction, unknowns, and gaps that need stakeholder resolution.
   - `## Sources` — a manifest of every file read from `.\references\` with a one-line summary.
6. **Stop and report.** After writing the draft, summarize: tier chosen, number of functional requirements extracted, number of NFRs quantified, open questions flagged, and any files that could not be read. Do not proceed to architecture or implementation — the draft is the sole deliverable for this phase.
